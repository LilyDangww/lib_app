  ...
  switch (normalized) {
    ...
    case "returned":
      return {
        ...
        type: "returned",
        action: returnDate
          ? `Trả ngày ${formatDateDisplay(returnDate)}`
          : "Đã hoàn thành",
      };
    case "lost":
...
const buildRentedBookItem = (raw: any): RentedBookItem => {
  const { label, type, action } = mapBorrowStatus(
    raw?.status,
    raw?.due_date,
    raw?.return_date
  );
  const overdueDays =
    type === "overdue" ? calculateOverdueDays(raw?.due_date) : null;

  return {
    id: String(
      raw?.borrow_detail_id ??
        raw?.borrow_id ??
...
export default function RentedBooks() {
    ...
    const fetchRentedBooks = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            payload?.message || "Không thể tải danh sách sách đã mượn."
          );
        }
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      <h1 className="text-2xl mb-6">Sách đã mượn</h1>
            ...
            {isLoading ? (
              ...
              paginatedBooks.map((book) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <div className="text-sm text-gray-600">{book.dueDate}</div>
                  </td>
                  <td className="py-3 px-4">
                    <span
